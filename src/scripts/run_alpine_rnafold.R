...
tissue <- "liver"
bam.files <- list(paste0("data/", sample_id, "/bam/Aligned.sortedByCoord.out.bam"))
bai <- paste0("data/", sample_id, "/bam/Aligned.sortedByCoord.out.bam.bai")
ensembldb_sqlite <- "data/Mus_musculus.GRCm38.102.gtf.sqlite"
transcript_file <- paste0("data/", tissue, "/training_set_transcripts.txt")
fragtypes_file <- paste0("data/", tissue, "/alpine_rnafold.fragtypes.rda")
BSgenome <- "BSgenome.Mmusculus.UCSC.mm10"
readlength <- 151
modelfile <- "temp.alpine_model.Rda"
